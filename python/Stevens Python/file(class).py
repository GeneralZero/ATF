...
class AnimeFile(object):
	...
	def moveWithMp3Tags(self,)


	def moveFiles(self, files):
			self.placenumber = self.name[self.filenumber].find(' - ')
			...
			if self.placenumber >= 0:
				...
				print self.com
				os.system(self.com)
				self.ensure_dir("/mnt/server/Music/" + self.files[:self.placenumber])
